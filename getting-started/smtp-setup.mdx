...
| Тип | Примеры | Отписка |
| --- | ------- | ------- |
| **Транзакционные** | Код входа, сброс пароля, чек, окончание подписки, результат автопродления | Не применяется |
| **Маркетинговые** | Winback, промопредложения, email-рассылки | Отписка обязательна |
...
# Пусто → CABINET_URL + /api/cabinet/public/unsubscribe
EMAIL_UNSUBSCRIBE_BASE_URL=
# Необязательный mailto-вариант в заголовке
...
SMTP_FROM_NAME=My VPN Service
SMTP_USE_TLS=false
# Куда падают ответы клиентов (опционально)
