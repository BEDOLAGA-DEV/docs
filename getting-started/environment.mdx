...
| Переменная | Описание | По умолчанию |
| ---------- | -------- | ------------ |
| `CHANNEL_SUB_ID` | ID канала (с `-100` префиксом) | — |
| `CHANNEL_IS_REQUIRED_SUB` | Обязательна ли подписка | `false` |
| `CHANNEL_LINK` | Ссылка на канал | — |
| `CHANNEL_DISABLE_TRIAL_ON_UNSUBSCRIBE` | Отключать триал при отписке | `true` |
| `CHANNEL_REQUIRED_FOR_ALL` | Требовать для ВСЕХ (платных и триальных) | `false` |
