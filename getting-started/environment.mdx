...
| `WEB_API_MANUAL_DEPOSIT_MAX_KOPEKS` | Потолок одной операции ручного пополнения через `POST /users/{id}/deposit` (копейки). `0` — без ограничения | `1000000` |

<Note>
  Эндпоинт ручного пополнения (`v3.67.0`) рассчитан на автоматизацию — например, на AI-агента поддержки. Лимит `WEB_API_MANUAL_DEPOSIT_MAX_KOPEKS` — предохранитель: агент, ошибшийся на два нуля, упрётся в него. Каждое начисление идемпотентно по внешнему ключу и проходит весь пост-топап конвейер (уведомления, автопокупка из корзины, реферальные бонусы).
</Note>
...
| `ALLOW_DEVICES_BELOW_TARIFF_LIMIT` | Разрешить опускать лимит устройств ниже включённого в тариф. `false` — нижняя граница равна `device_limit` тарифа, `true` — прежнее поведение (можно уменьшать до 1) | `false` |

<Note>
  `device_limit = 0` в тарифе означает **безлимит**, а не «ноль устройств» — так это и показывается пользователю и в админке (`v3.66.0`).
</Note>
...
| `PLATEGA_RECURRENT_ENABLED` | Автопродление списанием с карты через СБП-подписку Platega | `false` |
| `LAVA_RECURRENT_ENABLED` | Автопродление через рекуррентные подписки Lava | `false` |
| `RECURRENT_PAYMENTS_DISPLAY_MODE` | Где показывать документ «Рекуррентные платежи»: `bot`, `web`, `both`, `none` | `both` |

<Note>
  Автопродление с баланса (`ENABLE_AUTOPAY`) и провайдерские рекурренты (Platega, Lava) взаимоисключающи для одной подписки: включение одного выключает другое. Подробнее: [Рекуррентные платежи](/bot/recurrent-payments).

  Рекуррент Lava оформляется на **продукт** из кабинета Lava (цена и период заданы там), поэтому тарифу нужно проставить `lava_product_id`.
</Note>
...
## Grace access (ограниченный доступ после истечения)

Временный урезанный доступ для подписок в статусах `expired` и `limited` — чтобы человек успел продлиться, не оставаясь полностью без связи. Подробнее: [Grace access](/bot/grace-access).

| Переменная | Описание | По умолчанию |
| ---------- | -------- | ------------ |
| `GRACE_ACCESS_MODE` | `false` — выключено, `observe` — только журнал, `true` — выдавать доступ, `drain` — новые не выдаются, открытые доживают | `false` |
| `GRACE_ACCESS_DURATION_HOURS` | Длительность окна (часы) | `72` |
| `GRACE_ACCESS_TRAFFIC_GB` | Квота трафика на время grace (ГиБ) | `1` |
| `GRACE_ACCESS_EXPIRED_SQUAD_UUID` | UUID сквада для истёкших подписок | — |
| `GRACE_ACCESS_LIMITED_SQUAD_UUID` | UUID сквада для подписок с исчерпанным трафиком | — |
| `GRACE_ACCESS_TRIAL_ENABLED` | Выдавать grace триальным подпискам | `false` |
| `GRACE_ACCESS_DAILY_ENABLED` | Выдавать grace суточным тарифам | `false` |
| `GRACE_ACCESS_FREE_ENABLED` | Выдавать grace бесплатным (0₽) тарифам | `false` |
| `GRACE_ACCESS_RECONCILE_INTERVAL_SECONDS` | Интервал сверки сессий (сек) | `60` |
| `GRACE_ACCESS_RECONCILE_BATCH_SIZE` | Размер батча сверки | `200` |
| `GRACE_ACCESS_CANDIDATE_LOOKBACK_MINUTES` | Окно поиска кандидатов назад по времени (мин) | `30` |

<Note>
  При `GRACE_ACCESS_MODE=true` доступ получают обычные платные несуточные подписки. Триальные, суточные и бесплатные — только если включены соответствующие флаги.
</Note>
...
<Warning>
  Кнопки быстрых сумм пополнения задаются **per-метод в базе** через админ-панель бота (раздел **💸 Быстрые суммы**) и Cabinet Admin API, а не в `.env`. Переменных `DISABLE_TOPUP_BUTTONS`, `HIDE_TOPUP_BUTTONS` и `YOOKASSA_QUICK_AMOUNT_SELECTION_ENABLED` в боте не существует — раньше они ошибочно упоминались здесь. Подробнее: [Кнопки быстрых сумм](/bot/payments#кнопки-быстрых-сумм).
</Warning>
...
| `YOOKASSA_TRUSTED_PROXY_NETWORKS` | Доверенные **прокси перед ботом** (CIDR через запятую) — не allowlist отправителя | — |
| `YOOKASSA_SKIP_IP_CHECK` | Отключить проверку IP отправителя webhook; подлинность платежа подтверждается запросом в API YooKassa (fail-closed) | `false` |
...
<Warning>
  `YOOKASSA_TRUSTED_PROXY_NETWORKS` — это адреса **вашего** реверс-прокси или балансировщика, чьему заголовку `X-Forwarded-For` можно верить при вычислении реального IP отправителя. Приватные сети и диапазоны Cloudflare доверяются автоматически. IP самой YooKassa захардкожены в коде и сюда вписывать не нужно.

  `YOOKASSA_SKIP_IP_CHECK` (`v3.65.0`) нужен, когда бот стоит за Anti-DDoS, который не пробрасывает IP клиента — до бота доходит только адрес прокси, и вебхуки отбиваются с `forbidden_ip`. При включении каждый платёж обязательно подтверждается запросом в `api.yookassa.ru`: без исходящего доступа туда платежи не будут зачисляться вообще.
</Warning>
...
| `PLATEGA_API_VERSION` | Версия API создания платежа: `v1` — `/transaction/process`, `v2` — `/v2/transaction/process` | `v1` |
| `PLATEGA_RECURRENT_ENABLED` | Рекуррентные СБП-подписки Platega (автопродление) | `false` |

<Tip>
  Если карты (метод `11`) падают с ошибкой **No available card cascades** — переключите `PLATEGA_API_VERSION=v2`.

  Про СБП-автопродление см. [Рекуррентные платежи](/bot/recurrent-payments).
</Tip>
...
| Переменная | Описание | По умолчанию |
| ---------- | -------- | ------------ |
| `CISPAY_ENABLED` | Включить cisPay | `false` |
| `CISPAY_SHOP_ID` | `X-Shop-ID` — UUID магазина | — |
| `CISPAY_API_KEY` | `X-Api-Key` — секретный ключ (`cis_sec_...`) | — |
| `CISPAY_BASE_URL` | URL API | `https://api.cispay.app` |
| `CISPAY_DISPLAY_NAME` | Название в интерфейсе | `CisPay` |
| `CISPAY_CURRENCY` | Валюта | `RUB` |
| `CISPAY_MIN_AMOUNT_KOPEKS` | Мин. сумма (копейки) | `10000` |
| `CISPAY_MAX_AMOUNT_KOPEKS` | Макс. сумма (копейки) | `10000000` |
| `CISPAY_PAYMENT_LIFETIME_MINUTES` | Время жизни счёта (мин) | `30` |
| `CISPAY_WEBHOOK_PATH` | Путь webhook | `/cispay-webhook` |
| `CISPAY_CARD_ENABLED` | Отдельная кнопка «Карта» | `false` |
| `CISPAY_CARD_DISPLAY_NAME` | Название кнопки карты | `Карта (CisPay)` |
| `CISPAY_SBP_ENABLED` | Отдельная кнопка «СБП» | `false` |
| `CISPAY_SBP_DISPLAY_NAME` | Название кнопки СБП | `СБП (CisPay)` |
...
| `TRAFFIC_EXCLUDED_USER_IDS` | Числовые id панельных пользователей для исключения (через запятую) | — |

<Warning>
  **Изменение в v4.0.0.** Переменная `TRAFFIC_EXCLUDED_USER_UUIDS` переименована в `TRAFFIC_EXCLUDED_USER_IDS`, и изменился формат значений: вместо UUID панельных пользователей нужны **числовые id**. Автоматически сконвертировать нельзя — в Remnawave 3.0.0 прежних UUID больше не существует. Перевыпишите список заново из панели. Подробнее: [Обновление на Remnawave 3.0.0](/integrations/remnawave#обновление-на-remnawave-300).
</Warning>
...
| `MONITORING_NOTIFICATION_SEND_TIMEOUT` | Таймаут одной отправки уведомления из цикла мониторинга (сек) | `20.0` |

<Note>
  `MONITORING_NOTIFICATION_SEND_TIMEOUT` ограничивает **каждую** отправку по отдельности. Без него один зависший запрос к Telegram (до 60 секунд на получателя) тормозил весь цикл мониторинга на минуты.
</Note>
...
| `ADMIN_NOTIFICATIONS_RICH_ENABLED` | Rich-вид сообщений админ-чата (Bot API 10.1): заголовки, таблицы, сворачиваемые трейсбеки | `true` |

<Tip>
  `ADMIN_NOTIFICATIONS_RICH_ENABLED` намеренно закомментирован в `.env.example` — так настройка остаётся редактируемой из админки на лету. Значение, заданное в `.env`, блокирует её изменение через интерфейс.
</Tip>

### Шаблоны уведомлений о бане

Тексты, которые получает пользователь при бане. Причина приходит от [BedolagaBan](/bot/maintenance#система-бан-интеграции-bedolagaban), и под каждую есть свой шаблон (`v4.1.0`).

| Переменная | Когда отправляется |
| ---------- | ------------------ |
| `BAN_MSG_REVOKE` | Ключи доступа обновлены |
| `BAN_MSG_TORRENT` | Торренты |
| `BAN_MSG_HWID_LIMIT` | Превышен лимит устройств |
| `BAN_MSG_SUSPICIOUS_DESTINATION` | Подозрительное направление трафика |
| `BAN_MSG_TRAFFIC_LIMIT` | Превышен лимит трафика |
| `BAN_MSG_MANUAL` | Ручной бан администратором |

<Note>
  Шаблоны поддерживают HTML-разметку Telegram. Общий выключатель уведомлений (`ENABLE_NOTIFICATIONS`) действует и на них — при выключенных уведомлениях типизированные баны не отправляются.
</Note>
...
### Rich-главное меню (Bot API 10.1)

Главное меню с заголовками, таблицей подписок, сворачиваемыми блоками акций и датами в часовом поясе пользователя. Подробнее: [Настройки интерфейса](/bot/interface-settings#rich-главное-меню).

| Переменная | Описание | По умолчанию |
| ---------- | -------- | ------------ |
| `MAIN_MENU_RICH_ENABLED` | Включить rich-меню | `true` |
| `MAIN_MENU_RICH_EFFECT_ID` | Эффект сообщения при отправке меню (пусто — без эффекта) | `5046509860389126442` (🎉) |
| `MAIN_MENU_RICH_LOGO_URL` | Публичный HTTPS-URL логотипа в шапке. Пусто — авто-режим, `none` — меню без логотипа | — |
| `MAIN_MENU_RICH_SUBSCRIPTIONS_COLLAPSIBLE` | Сворачивать таблицу подписок при более чем одной подписке | `true` |

<Warning>
  Требуется `telegram-bot-api` с поддержкой Bot API 10.1+ (официальный сервер поддерживает). Если возможности недоступны, бот сам откатывается на классический рендер до перезапуска.

  Ключи `MAIN_MENU_RICH_*` в `.env.example` закомментированы намеренно — так они остаются редактируемыми из админки. Известные id эффектов: 🎉 `5046509860389126442`, ❤️ `5044134455711629726`, 🔥 `5104841245755180586`.
</Warning>

### Кнопка «Меню» с веб-кабинетом

Нижняя кнопка **Меню** в Telegram открывает веб-кабинет как WebApp. Бот при этом продолжает работать обычными сообщениями и inline-кнопками.

| Переменная | Описание | По умолчанию |
| ---------- | -------- | ------------ |
| `MENU_BUTTON_WEBAPP_ENABLED` | Включить WebApp на кнопке меню | `false` |
| `MENU_BUTTON_WEBAPP_TEXT` | Текст кнопки | `Кабинет` |
| `MENU_BUTTON_WEBAPP_URL` | URL кабинета (только `https`). Пусто — берётся `MINIAPP_CUSTOM_URL` | — |

<Note>
  При `false` существующая кнопка меню не трогается — отката к `MenuButtonDefault` не происходит. Настройка применяется при старте бота.
</Note>
...
| `HAPP_CRYPTOLINK_LOCAL_ENCRYPTION_ENABLED` | Локальная генерация crypt-ссылок (`happ://crypt4/...`) | `true` |
| `HAPP_CRYPTOLINK_API_FALLBACK_ENABLED` | Запасной путь через официальный Happ API (`crypto.happ.su` → `happ://crypt5/...`) | `true` |

<Note>
  Remnawave 2.8.0 удалил серверную генерацию crypt-ссылок (`/api/system/tools/happ/encrypt`). Начиная с `v3.63.0` бот шифрует ссылки **локально** публичным RSA-ключом Happ — так же, как это делает subpage панели. Внешний Happ API остаётся запасным путём.

  Выключатели нужны на крайний случай: `HAPP_CRYPTOLINK_LOCAL_ENCRYPTION_ENABLED=false` — если Happ ротирует ключ и до обновления бота ссылки нужно гнать через API; `HAPP_CRYPTOLINK_API_FALLBACK_ENABLED=false` — если не хотите обращений к внешнему сервису вообще (тогда crypt-ссылки будут только у тех, у кого они уже сохранены в БД).
</Note>
...
| `CABINET_REQUIRE_LEGAL_CONSENT` | Требовать согласие с офертой и политикой при **первой** авторизации новых пользователей | `true` |
| `CABINET_LEGAL_CONSENT_PRECHECKED` | Показывать чекбоксы согласия уже отмеченными | `false` |
| `SUPPORT_EMAIL` | Публичный email поддержки — отдаётся кабинетом в `GET /info/service` | — |
| `SERVICE_WEBSITE_URL` | Публичный сайт сервиса — отдаётся кабинетом в `GET /info/service` | — |
| `USER_ACTION_LOG_ENABLED` | Лог действий пользователя (кнопки в боте + действия в кабинете) для вкладки «Активность» в карточке юзера | `true` |
| `USER_ACTION_LOG_RETENTION_DAYS` | Сколько дней хранить лог действий | `90` |

<Note>
  Гейт согласия (`v3.67.0`) сам собой отключается, если ни оферта, ни политика не включены для веба. `CABINET_LEGAL_CONSENT_PRECHECKED=true` юридически слабее явной галочки, поэтому по умолчанию выключен.

  `USER_ACTION_LOG_*` в `.env.example` закомментированы — так они остаются редактируемыми из админки.
</Note>
...
| `SMTP_REPLY_TO` | Адрес в заголовке `Reply-To`. Пусто — заголовок не ставится | — |

<Tip>
  `SMTP_REPLY_TO` (`v4.1.0`) нужен, когда `SMTP_FROM_EMAIL` живёт на поддомене без MX-записи — типичная схема Resend и SES (`noreply@mail.example.com`). Ответ клиента на такое письмо просто отбивается, и обращение до вас не доходит.
</Tip>

### Отписка от маркетинговых писем

One-click unsubscribe по RFC 8058 — заголовок `List-Unsubscribe` и ссылка в футере (`v4.1.0`). Касается только маркетинга: winback, промопредложений и email-рассылок. Транзакционные письма (код входа, чек, окончание подписки) отписке не подлежат.

| Переменная | Описание | По умолчанию |
| ---------- | -------- | ------------ |
| `EMAIL_UNSUBSCRIBE_ENABLED` | Включить отписку | `true` |
| `EMAIL_UNSUBSCRIBE_BASE_URL` | Публичный URL эндпоинта отписки. Пусто — `CABINET_URL` + `/api/cabinet/public/unsubscribe` | — |
| `EMAIL_UNSUBSCRIBE_MAILTO` | Необязательный `mailto:`-вариант в заголовке `List-Unsubscribe` | — |
