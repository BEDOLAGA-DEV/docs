...
С `v4.1.0` `redirect_uri` формируется под **домен запроса**, а не жёстко из `CABINET_URL`. Благодаря этому вход через OAuth работает на зеркалах кабинета, а не только на основном домене. То же касается привязки провайдера к существующему аккаунту — она возвращается на тот домен, с которого началась.

<Note>
  Каждый домен-зеркало нужно отдельно добавить в список разрешённых Redirect URI в настройках провайдера — иначе провайдер отклонит запрос.
</Note>
