...
Бот поддерживает 17 платёжных провайдеров одновременно. Каждый провайдер включается отдельно, все работают параллельно через единый веб-сервер на порту `8080`.
...
17. RollyPay
18. AuraPay
19. Через поддержку (всегда последний)
