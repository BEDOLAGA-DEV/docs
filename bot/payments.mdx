...
Бот поддерживает 18 платёжных провайдеров одновременно. Каждый провайдер включается отдельно, все работают параллельно через единый веб-сервер на порту `8080`.
...
18. AuraPay
19. Overpay
20. Через поддержку (всегда последний)
