...
Бот автоматически передаёт в поле `client` запроса на создание платежа контакт плательщика — MulenPay использует его, чтобы иметь возможность связаться с клиентом по вопросам платежа. Значение выбирается по приоритету: email пользователя → телефон (если известен) → Telegram ID. Отдельная настройка не требуется.
