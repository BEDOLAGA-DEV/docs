...
Apple IAP — нативные покупки через iOS App Store (Mini App). Используется **App Store Server API** с JWT-аутентификацией (ES256, .p8 ключ). Поддерживает sandbox + production окружения, refund handling с защитой от double-deduction.
...
# или путь к файлу:
APPLE_IAP_PRIVATE_KEY_PATH=/path/to/AuthKey_2X9R4HXF34.p8
APPLE_IAP_ENVIRONMENT=production  # или sandbox
```
...
| Переменная | Тип | По умолчанию | Описание |
| --- | --- | --- | --- |
| `APPLE_IAP_ENABLED` | bool | false | Включает Apple IAP |
| `APPLE_IAP_BUNDLE_ID` | str | — | **Обязательно**. Bundle ID iOS-приложения |
| `APPLE_IAP_ISSUER_ID` | str | — | **Обязательно**. Issuer ID из App Store Connect |
| `APPLE_IAP_KEY_ID` | str | — | **Обязательно**. Key ID из App Store Connect |
| `APPLE_IAP_PRIVATE_KEY` | str | — | **Обязательно (или PATH)**. Содержимое .p8 файла |
| `APPLE_IAP_PRIVATE_KEY_PATH` | str | — | Альтернатива: путь к .p8 файлу |
| `APPLE_IAP_ENVIRONMENT` | str | production | `production` или `sandbox` |
| `APPLE_IAP_DISPLAY_NAME` | str | Apple In-App Purchase | Имя в кнопке |

### Refund handling

При получении server notification от Apple о возврате:
1. Транзакция блокируется через `FOR UPDATE` lock на `apple_transactions` row
2. Сумма списывается с баланса пользователя
3. Транзакция помечается как `refunded` (sticky)
4. Защита от double-deduction: повторный refund-webhook игнорируется

<Note>
  Apple IAP отображается **только в Mini App на iOS**. На Android и веб-кабинете кнопка не появляется.
</Note>

<Warning>
  Для подключения нужно создать App Store Server API key в App Store Connect (Users and Access → Keys). Bundle ID должен совпадать с реальным iOS-приложением.
</Warning>
...
| Apple IAP | `APPLE_IAP_ENABLED` + `BUNDLE_ID` + `ISSUER_ID` + `KEY_ID` + (`PRIVATE_KEY` или `PRIVATE_KEY_PATH`) |
