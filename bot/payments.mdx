...
Бот поддерживает 16 платёжных провайдеров одновременно. Каждый провайдер включается отдельно, все работают параллельно через единый веб-сервер на порту `8080`.
...
16. PayPear
17. RollyPay
18. Через поддержку (всегда последний)
