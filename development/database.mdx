...
| `PaymentMethod` | `telegram_stars`, `tribute`, `yookassa`, `cryptobot`, `heleket`, `mulenpay`, `pal24`, `wata`, `platega`, `cloudpayments`, `freekassa`, `kassa_ai`, `riopay`, `severpay`, `manual`, `balance` |
...
| `riopay_payments` | RioPay | `riopay_payment_id` |
| `severpay_payments` | SeverPay | `severpay_payment_id` |
...
| Группа полей | Ключевые поля |
| ------------ | ------------- |
| Основные | `id`, `title`, `slug` (UNIQUE), `content` (HTML), `excerpt` |
| Категория | `category` (legacy string), `category_color`, `category_id` (FK → news_categories, SET NULL) |
| Тег | `tag` (legacy string), `tag_id` (FK → news_tags, SET NULL) |
| Медиа | `featured_image_url` (http/https) |
| Публикация | `is_published`, `is_featured` (единственная), `published_at` |
| Метаданные | `read_time_minutes` (1-60), `views_count`, `created_by` (FK → users) |

Индексы: slug (unique), composite `is_published + published_at DESC`, composite `is_published + category`, `created_at`, FK-индексы.

**`news_categories`** — управляемые категории.

| Поле | Описание |
| ---- | -------- |
| `name` | Название (unique, case-insensitive) |
| `color` | HEX-цвет (`#00e5a0` по умолчанию) |

**`news_tags`** — управляемые теги.

| Поле | Описание |
| ---- | -------- |
| `name` | Название (unique, case-insensitive) |
| `color` | HEX-цвет (`#94a3b8` по умолчанию) |
...
| Новости | 3 | news_articles, news_categories, news_tags |
