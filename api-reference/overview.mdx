---
title: "Обзор API"
description: "Полное REST API бота Bedolaga — 710 эндпоинтов, 898 схем данных, 85 групп"
---
...
| Мобильная поддержка | JWT или Telegram `initData` | Живая переписка в тикетах, субпротокол `bedolaga.support.mobile.v1` |

<Note>
  По вебсокету кабинета приходят и события рекуррентных СБП-подписок Platega (`sbp_recurring.*`), и события тикетов (`ticket.created`, `ticket.status_changed`, `message.created`).
</Note>
...
<AccordionGroup>
  <Accordion title="Пользователи и подписки">
    | Группа | Описание |
    | ------ | -------- |
    | `users` | Управление пользователями, включая ручное пополнение баланса |
    | `subscriptions` | Подписки и ключи подключения |
    | `transactions` | Финансовые операции |
    | `partners` | Партнёрская программа |

    **Ручное пополнение** (`v3.67.0`): `POST /users/{id}/deposit` начисляет сумму на баланс и проводит её через весь пост-топап конвейер — уведомления, автопокупку из корзины, реферальные бонусы. Операция идемпотентна по внешнему ключу, повтор с тем же ключом деньги не удваивает.

    Потолок одной операции задаётся `WEB_API_MANUAL_DEPOSIT_MAX_KOPEKS` (по умолчанию 10 000 ₽, `0` — без ограничения). Эндпоинт рассчитан на автоматизацию — например, на AI-агента поддержки, — и лимит служит предохранителем от ошибки на два порядка.
  </Accordion>
